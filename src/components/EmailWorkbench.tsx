...
import { importedEmailThreads, type EmailStatus } from "@/lib/mock-data";
...
export function EmailWorkbench() {
  const [activeThreadId, setActiveThreadId] = useState(importedEmailThreads[0]?.id ?? "");

  const activeThread = useMemo(
    () => importedEmailThreads.find((thread) => thread.id === activeThreadId) ?? importedEmailThreads[0],
    [activeThreadId],
  );
    ...
    <section className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-3xl font-bold text-slate-950">Imported Email Workbench</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
            Imported correspondence viewer for pasted emails and EML archives. This is
            mock structure only, designed to show how threads will be recreated later.
          </p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
        <aside className="card min-h-[36rem] p-3">
          ...
          <div className="mt-3 space-y-2">
            {importedEmailThreads.map((thread) => {
        ...
        <article className="card min-h-[36rem] p-4">
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-4">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <StatusBadge tone={statusTone[activeThread.status]}>{activeThread.status}</StatusBadge>
              </div>
              <p className="mt-2 text-sm text-slate-600">
                Imported thread recreated from correspondence archive, not a live email client.
              </p>
            </div>
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            ...
            <MetaRow icon={Building2} label="Vessel / project" value={activeThread.vesselProject} />
            <MetaRow
              icon={FolderOpen}
              label="Linked case"
            ...
            <MetaRow label="CC" value={activeThread.cc.length > 0 ? activeThread.cc.join("; ") : "None"} />
          </div>
          ...
          <section className="mt-5">
            ...
            <div className="mt-3 space-y-3">
              {activeThread.messages.map((message) => (
                <article key={`${message.sender}-${message.timestamp}`} className="rounded-md border border-slate-200 bg-slate-50 p-3">
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-2">
            ...
            <Callout title="Thread note" value="Linked correspondence and draft workflow will be added in a later sprint." />
          </div>
        </article>
      </div>
    </section>
...
function MetaRow({
  ...
  return (
    <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
      ...
      <p
        className={`mt-2 text-sm leading-6 ${placeholder ? "font-semibold text-slate-500" : "text-slate-800"}`}
      >
        {value}
      </p>
    </div>
